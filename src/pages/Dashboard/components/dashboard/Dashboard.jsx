import React from 'react';
import { Nav, Navbar, Container, NavDropdown} from 'react-bootstrap';
import { Link } from 'react-router-dom';
import './dashboard.css';

export const Dashboard = () => {
  const blogUser = JSON.parse(localStorage.getItem('blogUser'));
  // console.log(blogUser)
  return (
    <div className="dashbaordMain">
      <header className="dashboardHeader">
      <Navbar bg="dark" variant="dark">
        <Container className="navWrapper">
          <Navbar.Brand href="#home">Tenda Africa Dashboard</Navbar.Brand>

         <div>
          <Nav className="me-auto" activeKey="/home">
              {/* <Nav.Item>
                  <NavDropdown eventkey="/link-1" href="/home" title="All Posts" id="navbarScrollingDropdown">
                    <NavDropdown.Item href="#action3">Technology</NavDropdown.Item>
                    <NavDropdown.Item href="#action4">Business</NavDropdown.Item>
                    <NavDropdown.Item href="#action4">Events</NavDropdown.Item>
                    <NavDropdown.Item href="#action4">Products</NavDropdown.Item>
                    <NavDropdown.Item href="#action4">Stories</NavDropdown.Item>
                    <NavDropdown.Item href="#action4">News</NavDropdown.Item>
                    <NavDropdown.Item href="#action4">Tutorials</NavDropdown.Item>
                  </NavDropdown>
              </Nav.Item> */}

              <Nav.Item>
                <Nav.Link href="dashboard/userposts" eventKey="link-2">All Posts</Nav.Link>
              </Nav.Item>

              <Nav.Item>
                <Nav.Link href="/dashboard/addpost" eventKey="link-3">Posts</Nav.Link>
              </Nav.Item>

              <Nav.Item>
                <Nav.Link href="/dashboard/adduser" eventKey="link-4">Users</Nav.Link>
              </Nav.Item>

              <Nav.Item>
                <Nav.Link href="/dashboard/addjob" eventKey="link-5">Jobs</Nav.Link>
              </Nav.Item>
            </Nav>
         </div>

          <div className="userDetails">
            <div className="userName">
              <span className="bloguserName">{blogUser.username}</span>
            </div>

            <div className="userPP">
              <img className="profileP" src="data:image/jpeg;base64," alt="" />
            </div>
           
          </div>
        </Container>
      </Navbar>
      

      </header>
    </div>
  )
}
